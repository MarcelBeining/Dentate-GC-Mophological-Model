function pts_IML = giveIMLpoints(N_IML,IMLdens,Somata,thicknesses,pts_ML,shapeprofile)
show = 0;
% fac = 2;
% while 1
...
% N_IML = max(N_IML,nclusters); % have at least as many iml points as clusters
if N_IML > 0
    [~, z] = find(diff(cat(2,zeros(N_IML,1),repmat(rand(N_IML,1),1,numel(IMLdens)) <= repmat(IMLdens,N_IML,1)),1,2));
    zindices = sum(thicknesses(1:2)):thicknesses(3)/(numel(IMLdens)-1):sum(thicknesses(1:3));  % scale IML thickness to IMLdens (normally 100 points)
    z = zindices(z);
    z((z-sum(thicknesses(1:2)))< 7) = sum(thicknesses(1:2))+7;  % 7µm Sperrzone sonst rutschen BPs in den Layer darunter mit smooth tree..
    scale = normrnd(0,0.1,1,N_IML); % calculate the random scaling of the centroid radius
    scale(abs(scale)>1) = 1;
    p = rand(1,N_IML);
