function pts_MML = giveMMLpoints(N_MML,MMLdens,Somata,thicknesses,pts_ML,shapeprofile)
show = 0;
% fac = 2;
% while 1
...
% N_MML = max(N_MML,nclusters); % have at least as many iml points as clusters
if N_MML > 0
    [~, z] = find(diff(cat(2,zeros(N_MML,1),repmat(rand(N_MML,1),1,numel(MMLdens)) <= repmat(MMLdens,N_MML,1)),1,2));
    zindices = sum(thicknesses(1:3)):thicknesses(4)/(numel(MMLdens)-1):sum(thicknesses(1:4));  % scale IML thickness to IMLdens (normally 100 points)
    z = zindices(z);
    z((z-sum(thicknesses(1:3)))< 7) = sum(thicknesses(1:3))+7;  % 7µm Sperrzone sonst rutschen BPs in den Layer darunter mit smooth tree..
    scale = normrnd(0,0.1,1,N_MML); % calculate the random scaling of the centroid radius
    scale(abs(scale)>1) = 1;
    p = rand(1,N_MML);
